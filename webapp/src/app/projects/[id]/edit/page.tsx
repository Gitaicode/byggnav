...
const projectSchema = z.object({
  ...
  num_buildings: z.coerce.number().int('Ange ett heltal').positive('Ange ett positivt tal').optional().nullable(),
    ...
    defaultValues: async (): Promise<ProjectFormData> => {
      ...
      try {
        ...
        const formData: ProjectFormData = {
            ...
            num_buildings: data.num_buildings ?? undefined,
        ...
        setImageUrl(data.building_image_url ?? null);
        return formData;
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white p-6 rounded shadow-md">
        <div className="space-y-4 p-4 border rounded-md">
            <h2 className="text-lg font-medium text-gray-900">Byggnadsbild</h2>
            {imageUrl ? (
                <div className="mb-4 relative w-full" style={{ paddingBottom: '56.25%' }}>
                    <Image 
                        src={imageUrl || ''}
                        alt="Nuvarande byggnadsbild"
                        layout="fill" 
                        objectFit="cover" 
                ...
                <p className="text-gray-500 italic mb-2">Ingen bild har laddats upp än.</p>
            )}
        ...
        <div>
          <label htmlFor="gross_floor_area" className="block text-sm font-medium text-gray-700">BTA (Bruttoarea) m²</label>
        ...
        <div>
          <label htmlFor="num_buildings" className="block text-sm font-medium text-gray-700">Antal byggnader</label>
          <input id="num_buildings" type="number" {...register('num_buildings')} className={`mt-1 block w-full px-3 py-2 border ${errors.num_buildings ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm`} />
          {errors.num_buildings && <p className="mt-1 text-sm text-red-600">{errors.num_buildings.message}</p>}
        </div>
        ...
        <div>
          <label htmlFor="tender_document_url" className="block text-sm font-medium text-gray-700">Förfrågningsunderlag</label>
          <input id="tender_document_url" type="url" {...register('tender_document_url')} className={`mt-1 block w-full px-3 py-2 border ${errors.tender_document_url ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm`} />
          {errors.tender_document_url && <p className="mt-1 text-sm text-red-600">{errors.tender_document_url.message}</p>}
        </div>

        <div>
          <label htmlFor="supplementary_tender_document_url" className="block text-sm font-medium text-gray-700">Kompletterande förfrågningsunderlag</label>
          <input id="supplementary_tender_document_url" type="url" {...register('supplementary_tender_document_url')} className={`mt-1 block w-full px-3 py-2 border ${errors.supplementary_tender_document_url ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm`} />
        ...
        <div>
          <label htmlFor="model_3d_url" className="block text-sm font-medium text-gray-700">Se 3D-modell i webbläsaren</label>
          <input id="model_3d_url" type="url" {...register('model_3d_url')} className={`mt-1 block w-full px-3 py-2 border ${errors.model_3d_url ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm`} />
          {errors.model_3d_url && <p className="mt-1 text-sm text-red-600">{errors.model_3d_url.message}</p>}
        </div>
